...
case 'buy': {
  ...
  try {
    ...
    let trx = await axios.post("https://passport.duitku.com/webapi/api/merchant/v2/inquiry", {
      ...
      paymentMethod: "SP", // metode pembayaran
...
// GANTIKAN CASE 'buypanel' LAMA ANDA DENGAN INI
case 'buypanel': {
    ...
    if (!q) {
      // Menggunakan template literal (tanda `) agar lebih mudah menulis teks multi-baris
...
👉 Ketik: \`/buypanel unli\``
      ...
      // Pastikan bot Anda mengirim pesan dengan parse_mode: "MARKDOWN"
      // Jika fungsi reply() Anda tidak mendukung opsi, gunakan fungsi sendText() atau sejenisnya
      reply(listMessage)
      break
    }
  ...
  if (cmd == "1gb" || cmd == "1") Obj = { ram: "1000", disk: "2000", cpu: "100", harga: 100 }
    ...
    else if (cmd == "9gb" || cmd == "9") Obj = { ram: "9000", disk: "5000", cpu: "200", harga: 6000 }
    ...
    try {
      ...
      let trx = await axios.post("https://passport.duitku.com/webapi/api/merchant/v2/inquiry", {
        ...
        merchantOrderId: merchantOrderId,
        productDetails: `Panel Pterodactyl ${cmd.toUpperCase().replace('GB','')}`, // Sedikit perbaikan agar tampilan lebih rapi
        customerVaName: pushname,
        email: username + "@telegram.com",
        phoneNumber: "08123456789", // Nomor placeholder
        itemDetails: [{
          name: `Panel Pterodactyl ${cmd.toUpperCase().replace('GB','')}`, // Sedikit perbaikan agar tampilan lebih rapi
          price: amount,
          quantity: 1
        }],
      ...
      await ronzz.sendPhoto({
        ...
        chat_id: user_id,
        caption: `*🧾 MENUNGGU PEMBAYARAN 🧾*\n\n` +
                 `*Produk:* Panel Pterodactyl ${cmd.toUpperCase().replace('GB','')}\n` + // Sedikit perbaikan agar tampilan lebih rapi
                 `*Harga:* Rp${toRupiah(amount)}\n\n` +
                 `📲 Scan QRIS atau tekan tombol di bawah untuk bayar\n`+
...
*┊・ ⚙️| Varian:* ${cmd.toUpperCase().replace('GB','')}
*┊・ 💰| Total Bayar:* Rp${toRupiah(amount)}
*┊・ 📅| Tanggal:* ${tanggal}
*┊・ ⏰| Jam:* ${jamwib} WIB
*╰┈┈┈┈┈┈┈┈*`, {
